...
async def get_crypto_info(start: int = 0, limit: int = 100) -> dict:
    try:
        st.session_state.console.append(
            f"Fetching crypto data from start={start} to limit={limit}")
        response = requests.get(
    ...
    trending = sorted(coins, key=lambda c: abs(
        float(c["percent_change_24h"])), reverse=True)[:30]
    return {
        "trending": [
            {
                "name": coin["name"],
...
def initialize_agent():
    return Agent(
        ...
        instructions="""
                ...
                try Keep responses to 2–3 lines if needed. Never just say "no action needed". If coin is not found, tell user and suggest using Coin ID do't talk about anything else except crypto.
                """,
